...
class ImageService {
    ...
    def thumbnail(InputStream is, OutputStream os, int maxWidth, int maxHeight) {
        Thumbnails.of(fixImage(is)).size(maxWidth, maxHeight).outputQuality(__DEFAULT_THUMBNAIL_QUALITY).rendering(Rendering.QUALITY).outputFormat('jpg').toOutputStream(os)
    }
